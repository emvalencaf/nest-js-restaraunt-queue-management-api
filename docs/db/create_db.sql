...
CREATE TABLE IF NOT EXISTS `restaurant_db`.`cancelled_reservation` (
  ...
  PRIMARY KEY (`reservation_id`),
  INDEX `cancelled_reservation_customer_id_idx` (`customer_id` ASC) VISIBLE,
  ...
  CONSTRAINT `cancelled_reservation_reservation_id`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `cancelled_reservation_customer_id`
    FOREIGN KEY (`customer_id`)
    REFERENCES `restaurant_db`.`customers` (`customer_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
BEGIN
    ...
    -- Only the reservation's customer or an employee can cancel the reservation
    IF NEW.customer_id IS NOT NULL THEN
        -- Check if the customer_id matches the reservation's customer_id
        SELECT EXISTS (
            SELECT 1
            FROM `restaurant_db`.`reservations`
            WHERE reservation_id = NEW.reservation_id 
              AND customer_id = NEW.customer_id
        ) INTO is_same_customer;

        IF NOT is_same_customer THEN
            SIGNAL SQLSTATE '45000' 
            SET MESSAGE_TEXT = 'Only the reservation\'s customer or an employee can cancel a reservation';
        END IF;
    END IF;
END$$
